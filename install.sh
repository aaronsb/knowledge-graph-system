...
setup_letsencrypt_dns() {
    ...
    if ~/.acme.sh/acme.sh --issue --dns "$SSL_DNS_PROVIDER" -d "$HOSTNAME" \
        --keylength 2048 \
        --cert-file "$(pwd)/certs/server.crt" \
        --key-file "$(pwd)/certs/server.key" \
        --fullchain-file "$(pwd)/certs/fullchain.crt"; then

        chmod 600 certs/server.key
        log_success "Certificate obtained successfully"
    else
        log_error "Failed to obtain certificate"
        log_info "Check ~/.acme.sh/acme.sh.log for details"
...
setup_letsencrypt_http() {
    ...
    if sudo certbot certonly --standalone \
        ...
        cp "$certbot_dir/fullchain.pem" certs/server.crt
        cp "$certbot_dir/privkey.pem" certs/server.key
        chmod 600 certs/server.key
...
setup_cert_renewal() {
    ...
    cat > "$install_dir/renew-certs.sh" << 'EOF'
#!/bin/bash
...
~/.acme.sh/acme.sh --renew -d "$HOSTNAME" \
    --cert-file "$(pwd)/certs/server.crt" \
    --key-file "$(pwd)/certs/server.key" \
    --fullchain-file "$(pwd)/certs/fullchain.crt" \
...
EOF
    ...
    echo "SSL_MODE=$SSL_MODE" >> "$install_dir/.operator.conf"
    if [[ "$MACVLAN_ENABLED" == "true" ]]; then
        echo "MACVLAN_ENABLED=true" >> "$install_dir/.operator.conf"
        [[ -n "$MACVLAN_IP" ]] && echo "MACVLAN_IP=$MACVLAN_IP" >> "$install_dir/.operator.conf"
    fi
}

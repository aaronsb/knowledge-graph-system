...
run_interactive_setup() {
    ...
    echo
    local save_choice
    save_choice=$(prompt_select "Save configuration for future installs?" \
        "yes (save to $CONFIG_FILE)" \
        "no")
...
server {
    ...
    location /api/ {
        ...
        # Backup restore (ADR-107) and document/image ingest stream multipart

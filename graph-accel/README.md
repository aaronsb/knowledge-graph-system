...
BFS from `start_id` up to `max_depth` hops. Returns all reachable nodes with their minimum distance and the relationship types along one shortest path from the start.

Traversal is **undirected** -- follows both incoming and outgoing edges. Each node appears at most once, at its minimum distance.
...
Finds the unweighted shortest path between two nodes. Returns the full path as ordered steps (0-indexed). Empty result set if no path exists within `max_hops` -- not an error.
...
graph_accel_status()
  RETURNS TABLE(
    source_graph   TEXT,
    status         TEXT,     -- 'loaded' or 'not_loaded'
    node_count     BIGINT,
    edge_count     BIGINT,
    memory_bytes   BIGINT,
    rel_type_count INT
...
Always works, even when no graph is loaded. Returns the current state of the per-backend graph.
...
| `graph_accel.auto_reload` | bool | true | *(Future)* Auto-reload when graph changes detected. |
| `graph_accel.reload_debounce_sec` | int | 5 | *(Future)* Minimum seconds between reloads. |
...
docker cp target/release/graph_accel-pg17/.../extension/graph_accel--0.1.0.sql \
  my-postgres-container:/usr/share/postgresql/17/extension/
...
postgres:
  ...
  volumes:
    ...
    - ./graph_accel--0.1.0.sql:/usr/share/postgresql/17/extension/graph_accel--0.1.0.sql:ro
```
...
| 3. Epoch invalidation | Planned | Stale detection, auto-reload, debouncing |

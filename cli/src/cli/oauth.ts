...
async function createClientCommand(options: { name?: string; for?: string }) {
  try {
    ...
    console.log(`\n🔐 Creating OAuth client${target !== 'generic' ? ` for ${target}` : ''}...\n`);
    ...
    if (target === 'mcp') {
      ...
      // kg-fuse reads credentials from ~/.config/kg/config.json (shared with CLI)
      // After 'kg login', kg-fuse picks up the credentials automatically
      console.log('\x1b[1mFUSE SETUP\x1b[0m');
      console.log('─'.repeat(60));
      console.log('  kg-fuse uses credentials from kg login.');
      console.log('  If you need a separate client for FUSE, set env vars:');
      console.log('');
      console.log(`    export KG_OAUTH_CLIENT_ID="${client.client_id}"`);
      console.log(`    export KG_OAUTH_CLIENT_SECRET="${client.client_secret}"`);
      console.log('');
      console.log('  Or initialize the mount:');
      console.log('    kg-fuse init /mnt/knowledge');
      console.log('');
    } else {
      // Generic usage

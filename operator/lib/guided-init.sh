...
# headless-init.sh and the standalone install.sh).
...
echo "  • Embeddings: Local (nomic-ai/modernbert-embed-base + SigLIP 2 image index)"
echo ""
...
echo ""
echo -e "  ${GREEN}[3] Linux with AMD GPU (ROCm)${NC}"
echo "      • Uses AMD's official rocm/pytorch base image (ROCm 7.x)"
echo "      • Requires host ROCm kernel driver + /dev/kfd, /dev/dri"
echo "      • Works with RX 5000/6000/7000 series and Instinct cards"
echo ""
echo -e "  ${GREEN}[4] CPU only${NC}"
...
# Persist platform configuration BEFORE starting infrastructure so that
# start-infra.sh sees the correct DEV_MODE (which controls whether the
# prod compose overlay is applied) and matches container names accordingly.
DEV_MODE="false"
...
# Edit with: ./operator.sh config --dev true --gpu nvidia|amd-host|mac|cpu
DEV_MODE=$DEV_MODE
GPU_MODE=$GPU_MODE
KG_API_IMAGE_TAG=$KG_API_IMAGE_TAG
INITIALIZED_AT=$(date -Iseconds)
EOF
echo -e "${GREEN}✓${NC} Configuration saved (dev=$DEV_MODE, gpu=$GPU_MODE, image_tag=$KG_API_IMAGE_TAG)"

# ADR-101: warn about multi-GPU coexistence on AMD ROCm hosts. When both a
# discrete dGPU and an iGPU are visible to ROCm, the runtime enumerates both
# and our embedding model lands on whichever it picks first — often the iGPU,
# which is far slower. ROCR_VISIBLE_DEVICES pins the runtime to one device.
if [ "$GPU_MODE" = "amd" ] || [ "$GPU_MODE" = "amd-host" ]; then
    echo ""
    echo -e "${YELLOW}ℹ️  AMD multi-GPU note:${NC} If your host has both a discrete AMD GPU and an"
    echo "   iGPU (e.g., Ryzen 7000+ with integrated graphics), set"
    echo "   ${BLUE}ROCR_VISIBLE_DEVICES=0${NC} in .env to pin embeddings to the discrete card."
    echo "   Run 'rocminfo | grep gfx' on the host to see device ordering."
fi
echo ""
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
cai_select_provider   # sets AI_PROVIDER

# Step 6: Store and validate the API key
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
# Start the platform using operator.sh (config was saved before Step 2)

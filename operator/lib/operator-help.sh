...
show_overview() {
    ...
    echo "  status                 View current platform configuration"
    echo "  pg                     Connect to database (credentials auto-loaded)"
    echo "  configure              Run configure.py"
    echo ""
...
show_database_help() {
    ...
    echo ""
    echo -e "${CYAN}Quick Access (credentials auto-loaded):${NC}"
    echo "  pg                     # Connect to database (alias for psql)"
    echo "  pg -c 'SELECT 1'       # Run single query"
    echo ""

...
class APIClient {
  ...
  /**
   * Get annealing loop health, configuration, and schedule.
   */
  async getAnnealingStatus(): Promise<AnnealingStatus> {
    const response = await this.client.get<AnnealingStatus>('/ontology/annealing/status');
    return response.data;
  }

  /**
   * List annealing proposals generated by the loop.
   */
  async listAnnealingProposals(
    filters?: AnnealingProposalFilters,
  ): Promise<AnnealingProposalListResponse> {
    const response = await this.client.get<AnnealingProposalListResponse>('/ontology/proposals', {
      params: {
        status: filters?.status,
        proposal_type: filters?.proposal_type,
        ontology: filters?.ontology,
  ...
  /**
   * Approve or reject a pending annealing proposal. Requires ontologies:write.
   */
  async reviewAnnealingProposal(
    proposalId: number,
    status: 'approved' | 'rejected',
    notes?: string,
  ): Promise<AnnealingProposal> {
    const response = await this.client.post<AnnealingProposal>(
      `/ontology/proposals/${proposalId}/review`,
      { status, notes },
    );
    return response.data;
  }
  ...
  /**
   * Trigger a manual annealing cycle. Requires ontologies:write.
   */
  async triggerAnnealingCycle(dryRun = false): Promise<AnnealingCycleResult> {
    const response = await this.client.post<AnnealingCycleResult>(

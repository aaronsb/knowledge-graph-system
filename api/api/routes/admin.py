        ...
        with open(temp_path, 'r') as f:
            ...
            if backup_type == "ontology_backup" and backup_ontology and overwrite:
                ...
                try:
                    ...
                    from api.api.routes.ontology import list_ontologies
...
async def regenerate_embeddings(
    ...
    embeddings, or bulk regeneration after embedding config changes.

    **Authentication:** Requires admin role

    Args:
        embedding_type: Type of embeddings to regenerate: 'concept', 'source', 'vocabulary', 'all'
        only_missing: Only generate for entities without embeddings (concept/source only)
    ...
    **Example Usage:**
    ...
    curl -X POST http://localhost:8000/admin/regenerate-embeddings?embedding_type=concept

    # Regenerate missing source embeddings only
    curl -X POST http://localhost:8000/admin/regenerate-embeddings?embedding_type=source&only_missing=true

    # Regenerate vocabulary embeddings
    curl -X POST http://localhost:8000/admin/regenerate-embeddings?embedding_type=vocabulary

    # Regenerate ALL embeddings (model migration)
    curl -X POST http://localhost:8000/admin/regenerate-embeddings?embedding_type=all
    ```
    """
    try:

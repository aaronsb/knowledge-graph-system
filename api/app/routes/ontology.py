...
async def list_proposals(
    ...
    """
    List breathing proposals (ADR-200 Phase 3b).

    Returns proposals generated by the breathing cycle for human review.
    """
    client = get_age_client()
    try:
...
@router.get("/proposals/{proposal_id}", response_model=BreathingProposal)
async def get_proposal(
    proposal_id: int,
    current_user: CurrentUser = None,
):
    """Get a single breathing proposal by ID."""
    client = get_age_client()
    try:
        conn = client.pool.getconn()
        try:
...
@router.post("/proposals/{proposal_id}/review", response_model=BreathingProposal)
async def review_proposal(
    proposal_id: int,
    request: BreathingProposalReviewRequest,
    current_user: CurrentUser = None,
...
@router.post("/breathing-cycle", response_model=BreathingCycleResult)
async def trigger_breathing_cycle(
    ...
    """
    Trigger a manual breathing cycle (ADR-200 Phase 3b).

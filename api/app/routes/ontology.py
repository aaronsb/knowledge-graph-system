...
async def list_proposals(
    ...
    """
    List annealing proposals (ADR-200 Phase 3b).

    Returns proposals generated by the annealing cycle for human review.
    """
    client = get_age_client()
    try:
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if ontology:
                    conditions.append("ontology_name = %s")
                ...
                return AnnealingProposalListResponse(
                    proposals=proposals,
                    count=len(proposals),
                )
        finally:
...
async def get_proposal(
    ...
    """Get a single annealing proposal by ID."""
    client = get_age_client()
    try:
        conn = client.pool.getconn()
        try:
        ...
        logger.error(f"Failed to get proposal: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        client.close()


@router.post("/proposals/{proposal_id}/review", response_model=AnnealingProposal)
async def review_proposal(
    proposal_id: int,
    request: AnnealingProposalReviewRequest,
    current_user: CurrentUser = None,
    _: None = Depends(require_permission("ontologies", "write")),
):
    """
    Approve or reject a annealing proposal (ADR-200 Phase 3b + Phase 4).
    ...
    **Authorization:** Requires `ontologies:write` permission
    ...
    try:
        ...
        logger.error(f"Failed to review proposal: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        client.close()


@router.post("/annealing-cycle", response_model=AnnealingCycleResult)
async def trigger_annealing_cycle(
    dry_run: bool = QueryParam(False, description="Identify candidates without LLM evaluation or proposal storage"),
    demotion_threshold: float = QueryParam(0.15, description="Protection score below which to consider demotion"),
    promotion_min_degree: int = QueryParam(10, description="Minimum concept degree for promotion candidacy"),
    max_proposals: int = QueryParam(5, ge=1, le=20, description="Maximum proposals per cycle"),
    current_user: CurrentUser = None,
    _: None = Depends(require_permission("ontologies", "write")),
):
    """
    ...
    **Authorization:** Requires `ontologies:write` permission
    ...
    # Auto-approve and start immediately
    ...
    # Wait briefly for fast cycles (up to 10s)
    for _ in range(20):
    ...
    if job and job.get("status") == "completed" and job.get("result"):
        result = job["result"]
        return AnnealingCycleResult(
            proposals_generated=result.get("proposals_generated", 0),
            demotion_candidates=result.get("demotion_candidates", 0),
            promotion_candidates=result.get("promotion_candidates", 0),
            scores_updated=result.get("scores_updated", 0),
            centroids_updated=result.get("centroids_updated", 0),
            cycle_epoch=result.get("cycle_epoch", 0),
            dry_run=result.get("dry_run", dry_run),
        )

    # Still running — return what we know
    return AnnealingCycleResult(
        cycle_epoch=0,
        dry_run=dry_run,
    )
...
async def get_ontology_scores(
    ...
    """
    Get cached annealing scores for an ontology (ADR-200 Phase 3a).
...
async def compute_ontology_scores(
    ...
    """
    Recompute and cache annealing scores for an ontology (ADR-200 Phase 3a).
...
async def compute_all_ontology_scores(
    ...
    """
    Recompute and cache annealing scores for all ontologies (ADR-200 Phase 3a).
...
async def get_ontology_edges(
    ...
    the annealing cycle or created manually.
...
async def create_ontology_edge(
    ...
    Manual edges persist across annealing cycles (source='manual').

...
# ============================================================================
# rebuild-web.sh
# Rebuild web UI image and restart container to pick up changes
# ============================================================================
...
echo -e "${BLUE}→ Rebuilding web UI image...${NC}"
...
# Rebuild web image
docker-compose --env-file "$PROJECT_ROOT/.env" build web

echo -e "${GREEN}✓ Web UI image rebuilt${NC}"
echo ""

# Check if container is running
if docker ps --format '{{.Names}}' | grep -q "^kg-web-dev$"; then
    echo -e "${YELLOW}→ Restarting web container to pick up new image...${NC}"
    docker-compose --env-file "$PROJECT_ROOT/.env" up -d --force-recreate --no-deps web
    echo -e "${GREEN}✓ Web container restarted${NC}"
else
    echo -e "${YELLOW}⚠  Web container not running${NC}"

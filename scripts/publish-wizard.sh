...
# ============================================================================
# Version is read from VERSION file at runtime (see get_wizard_version)
# ============================================================================
#
...
# --- Version (read from VERSION file to avoid drift) ---
get_wizard_version() {
    cat "$PROJECT_ROOT/VERSION" 2>/dev/null | tr -d '[:space:]' || echo "unknown"
}
...
prompt_multiselect() {
    ...
    echo -e "\n${CYAN}?${NC} Select artifacts to publish:" >&2
    echo -e "   ${DIM}(Enter 1-${#names[@]} to toggle, 'd' when done)${NC}" >&2

    while true; do
        echo "" >&2
        for i in "${!names[@]}"; do
            local checkbox="${RED}○${NC}"
            [[ "${selected[$i]}" == "true" ]] && checkbox="${GREEN}●${NC}"
            echo -e "   $((i+1))) $checkbox ${names[$i]}" >&2
        done
        echo -e "   ${DIM}d) Done${NC}" >&2

        local choice
        echo -ne "   > " >&2
        read -r choice </dev/tty

        if [[ "$choice" == "d" || "$choice" == "done" || -z "$choice" ]]; then
            break
            ...
            echo -e "   ${DIM}Invalid choice, try again${NC}" >&2
        fi
    done
...
# Generated by publish-wizard.sh $(get_wizard_version) on $(date -Iseconds)
...
load_config() {
    ...
    # Security: validate config contains only expected variable assignments
    # Reject if file contains anything other than comments and known variables
    local valid_pattern='^[[:space:]]*(#.*)?$|^[[:space:]]*(DEFAULT_BUMP_TYPE|PUBLISH_IMAGES|PUBLISH_CLI|PUBLISH_FUSE|USE_MULTI_ARCH)='
    if grep -vE "$valid_pattern" "$CONFIG_FILE" | grep -q '[^[:space:]]'; then
        log_warn "Config file contains unexpected content, ignoring"
        return 1
    fi
...
show_status() {
    ...
    # Show saved config if exists (uses validated load_config)
    if load_config; then
...
step_enter_message() {
    ...
    local trimmed="${RELEASE_MESSAGE//[$'\t\r\n ']/}"
    if [[ -z "$trimmed" ]]; then
        log_error "Release message cannot be empty"
        return 1
    fi
...
run_wizard() {
    ...
    show_wizard_art
    echo -e "${BOLD}Knowledge Graph Release Wizard v$(get_wizard_version)${NC}"

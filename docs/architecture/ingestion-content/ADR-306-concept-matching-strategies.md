...
3. **ADR-805 Configuration Pattern** - Database-first configuration with hot-reload
4. **ADR-505 Epsilon-Greedy Pattern** - Proven degree-biased search strategies
...
ADR-505 discovered that degree centrality bias improves both performance and quality for graph traversal. The same principle applies to concept matching:
...
Following ADR-505 epsilon-greedy pattern:
...
**Query Pattern with Degree Centrality (following ADR-505):**
```cypher
...
Following ADR-805 pattern (database-first with fallbacks):
...
✅ **Proven Pattern**
- Follows ADR-805 (database-first configuration)
- Applies ADR-505 lessons (degree-biased search)
- Consistent with platform architecture

✅ **Future-Ready**
- Evidence-aware matching (when ADR-812 complete)
...
1. Implement inline degree calculation (following ADR-505 pattern)
...
This ADR leaves room for a future Evidence-Aware Concept Matching ADR (in the
ingest band) which will add:
...
# Evidence similarity (future ADR)
...
This requires ADR-812 (Source Text Embeddings) to be implemented first.
...
**Cons:** Misses opportunity to apply ADR-505 lessons
...
**Decision:** Rejected - Embedding model separate concern (ADR-805)
...
- **[ADR-505: Parallel Graph Query Optimization](../query-search/ADR-505-parallel-graph-queries.md)** - Epsilon-greedy degree-biased search pattern; the planned standalone "ADR-505" findings document was never split out and lives inline in [Actual Performance Results](../query-search/ADR-505-parallel-graph-queries.md#actual-performance-results) — discovery that query optimization > parallelization

...
# ADR-304.2: API Changes for Graph-Based Document Deduplication

## Overview

ADR-304.1 introduced a fundamental architectural change: moving document deduplication from the ephemeral jobs table to permanent DocumentMeta nodes in the graph. But that decision requires actual API and database changes to implement. This document specifies exactly what needs to change and how to do it while maintaining backward compatibility with existing clients.
...
## Current API (Before ADR-304)

This document specifies the exact API changes needed to implement ADR-304. These changes are **backward compatible** - existing clients continue to work without modification.
...
# Full call with source metadata (ADR-304)
response = requests.post(
    "http://localhost:8000/ingest/text",
    data={
        "text": "Some content...",
...
### Before (ADR-300 - Jobs Table)

**Location:** `api/app/services/content_hasher.py`
...
```python
def check_duplicate(self, content_hash: str, ontology: str) -> Optional[Dict]:
    """Check graph for duplicate (ADR-304)"""
...
def get_document_meta(self, content_hash: str, ontology: str) -> Optional[Dict]:
    """Check if document exists in graph (ADR-304)"""
    query = """
    SELECT * FROM cypher('knowledge_graph', $$
...
```python
def check_duplicate(self, content_hash: str, ontology: str) -> Optional[Dict]:
    """Check graph first, jobs table second (ADR-304)"""

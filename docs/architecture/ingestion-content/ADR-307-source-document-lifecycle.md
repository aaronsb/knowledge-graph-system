...
**ADR-307 is an insurance policy against model obsolescence.** It allows replaying history with smarter agents later.
...
This is implemented via `ContentHasher` → `DocumentMeta` lookup (ADR-304).
...
Note: DocumentMeta (ADR-304) serves as the Document node type. No separate :Document label needed.
...
## Enables: Semantic FUSE Filesystem (ADR-715)

This ADR provides the final foundational piece for the Semantic FUSE Filesystem (ADR-715):

```
ADR-305 (Images in Garage)
    ↓
ADR-114 (Projections in Garage)
    ↓
ADR-307 (Source Documents in Garage) ← THIS ADR
    ↓
...
- ADR-715: Semantic FUSE Filesystem (enabled by this ADR)

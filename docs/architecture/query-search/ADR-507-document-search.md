...
For complete ontology cloning/export, use the existing backup system (ADR-107):

```bash
kg admin backup --type ontology --ontology "CS Research"
```

**Future enhancement (ADR-107 extension):**
...
### Neutral
- Builds on existing `:DocumentMeta` infrastructure (ADR-304)
- Complements concept search (different discovery patterns)
- Backup extends naturally for full ontology cloning (ADR-107)
...
- ADR-107: Backup/restore streaming (ontology-scoped export, extend for Garage content)
- ADR-304: Document deduplication and `:DocumentMeta` nodes
- ADR-305: Image storage with prose descriptions (image + prose file pairing)
- ADR-812: Source text embeddings (chunk-level search)
- ADR-307: Source document lifecycle and Garage storage

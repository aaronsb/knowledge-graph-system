...
**Date**: 2025-01-08
**Author**: System
**Related ADRs**: ADR-808 (Dynamic Grounding), ADR-809 (Unified Embedding), ADR-811 (Polarity Axis Triangulation)
...
Knowledge graphs built from LLM extraction can contain both authentic information (based on real-world observations from independent sources) and fabricated claims (variations on false narratives). While grounding strength (ADR-811) measures evidential support through relationship polarity, it doesn't capture the **richness and diversity** of supporting evidence.
...
- Calculated on-demand (like grounding strength in ADR-808)
...
This aligns with **signed graph theory** where edge polarity affects semantic propagation. The metric preserves the diversity magnitude while incorporating directionality from grounding (ADR-808 + ADR-811).
...
4. **Embedding Model Dependency**: Diversity scores are downstream of the embedding model
   ...
   - Recalculation required on model migration (handled by ADR-809 embedding worker)
...
- **Grounding Strength (ADR-811)**: Measures polarity (support/contradict) of evidence

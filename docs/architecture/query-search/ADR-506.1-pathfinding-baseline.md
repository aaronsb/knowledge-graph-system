...
# ADR-506.1: Pathfinding Performance Baseline

## Purpose

This document records baseline performance measurements for the current exhaustive pathfinding implementation. These measurements establish the "before" state against which the Bidirectional BFS optimization (ADR-506) will be compared.
...
5. **Bidirectional BFS justified:** The O(b^d) → O(b^(d/2)) improvement from ADR-506 is necessary for production use

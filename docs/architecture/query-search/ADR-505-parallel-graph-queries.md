...
### Database-First (Following ADR-805/049 Pattern)
...
def load_parallel_config() -> ParallelQueryConfig:
    """Load configuration from database (ADR-805/049 pattern)"""
    try:
...
- **[ADR-813: Polarity Axis Analysis](../ai-embeddings/ADR-813-polarity-axis-analysis.md)** - The feature that exposed this performance issue
- **[ADR-810: Rate Limiting and Concurrency](../ai-embeddings/ADR-810-rate-limiting-and-concurrency.md)** - Semaphore pattern for resource limiting
- **[ADR-606: GraphQueryFacade](../vocabulary-relationships/ADR-606-vocabulary-metadata-as-graph.md)** - Namespace-safe query interface

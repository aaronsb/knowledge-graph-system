...
**Recent Example:**
Adding `kg_api.embedding_config` table for ADR-804 required manual insertion into `init.sql`. If a developer has an existing database, they must:
...
-- Description: Add embedding configuration table for ADR-804
...
└── migrations/
    ├── 001_baseline.sql        # Current schema as of ADR-210
    ├── 002_add_embedding_config.sql
...
-- ADR: ADR-804 Local Embedding Service
-- Date: 2025-10-20
...
CREATE TABLE IF NOT EXISTS kg_api.embedding_config (
    -- ... (full table definition from ADR-804)
);
...
- **ADR-804:** Local Embedding Service (triggered need for migration system)
- **ADR-208:** Apache AGE Migration (major schema change that would have benefited from migrations)

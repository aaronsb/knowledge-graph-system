...
ADR-208 established PostgreSQL + Apache AGE as our unified database, giving us "relational SQL for free" alongside graph capabilities. However, the current implementation mixes all concerns into a single schema namespace (`public`), which creates several challenges:
...
From ADR-208, we already have PostgreSQL running for Apache AGE. The "dual-use benefit" means:
...
kg_api.ingestion_jobs (
    ...
    analysis JSONB,            -- Pre-ingestion cost estimates (ADR-300)
...
-- Relationship Vocabulary Management (ADR-601)
...
-- Edge Usage Stats & Performance Optimization (ADR-601)
...
- Loses "dual-use benefit" from ADR-208
...
- ADR-208: Apache AGE Migration (establishes PostgreSQL as unified database)
- ADR-300: Job Approval Workflow (job queue requirements)

...
- **ADR-106:** API Server Architecture (job queue, authentication)
- **ADR-707:** Unified TypeScript Client (configuration)
- **ADR-107:** Backup/Restore Streaming (simplified with pg_dump)
...
This unification directly impacts ADR-107 (Backup/Restore Streaming) by eliminating the need for custom backup logic - standard PostgreSQL tools handle everything.

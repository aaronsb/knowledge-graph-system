...
- Image storage (ADR-305)
- Projection storage (ADR-114)
- Credential management
- S3 client initialization

This violates the Single Responsibility Principle (#175) and makes the code harder to maintain and test.

This ADR focuses on **clean code refactoring** - establishing a modular service architecture that future features (like source document storage in ADR-307) can build upon.

**Scope clarification:**
- **ADR-115 (this)**: Service architecture, SRP refactor, retention policies
- **ADR-307**: Source document lifecycle, deduplication, versioning, regeneration
...
├── image_storage.py            # ImageStorageService (ADR-305)
├── projection_storage.py       # ProjectionStorageService (ADR-114)
├── source_storage.py           # SourceDocumentService (NEW)
└── retention.py                # RetentionPolicyManager (NEW)
```

Each service follows the **Facade pattern** established by `GraphQueryFacade` (ADR-606).
...
class ImageStorageService:
    """Image storage operations (ADR-305)."""
...
class ProjectionStorageService:
    """Projection artifact storage (ADR-114)."""
...
- ADR-606: GraphQueryFacade (similar pattern)
- ADR-305: Multimodal Image Ingestion
- ADR-114: Projection Artifact Storage

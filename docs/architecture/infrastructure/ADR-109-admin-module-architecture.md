...
5. **Database Migration**: Migration from Neo4j to Apache AGE (ADR-208) broke `reset.sh` which still referenced Neo4j containers
...
- **ADR-107**: Backup/Restore Streaming - Established pattern for admin modules
- **ADR-208**: Apache AGE Migration - Required reset.sh replacement

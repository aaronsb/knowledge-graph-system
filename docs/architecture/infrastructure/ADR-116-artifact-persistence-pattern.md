...
- Projections use Garage storage (ADR-114) but ad-hoc
...
3. **Ownership model** integrated with ADR-410
...
CREATE TABLE kg_api.artifacts (
    ...
    -- Ownership (NULL = system-owned, integrates with ADR-410)
    owner_id INTEGER REFERENCES kg_auth.users(id),
...
For expensive computations (>30 seconds), integrate with ADR-300 job queue:
...
### 10. Migration from ADR-114

ADR-114 established projection storage in Garage. This ADR generalizes that pattern:
...
3. **Shareable** - Artifacts can be shared via grants (ADR-410)
...
- ADR-300: Job Queue (async processing)
- ADR-114: Projection Artifact Storage (Garage pattern, superseded/generalized)

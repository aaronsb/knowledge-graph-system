...
The Embedding Landscape Explorer (ADR-717) computes t-SNE/UMAP projections of concept embeddings into 3D space. These projections:
...
1. **ADR-808 Contradiction**: Grounding strength must be calculated fresh per ADR-808's "always current" philosophy
...
We already have Garage (S3-compatible storage) set up for image ingestion (ADR-305). Projection artifacts are a natural fit:
...
1. **No ADR-808 Conflict**: Grounding/diversity still calculated fresh at query time
...
6. **Leverages Existing Infrastructure**: Reuses Garage from ADR-305
...
1. **Does NOT cache grounding_strength on Concept nodes** - Always calculated fresh per ADR-808
2. **Does NOT cache diversity_score on Concept nodes** - Always calculated fresh per ADR-503
...
- ADR-305: Multimodal Image Ingestion (Garage infrastructure)
- ADR-717: Embedding Landscape Explorer (projection computation)
- ADR-808: Probabilistic Truth Convergence (grounding calculation philosophy)
- ADR-503: Semantic Diversity (diversity calculation philosophy)

...
**Related**: ADR-107 (Backup/Restore Streaming), ADR-300 (Job Queue System)
...
After implementing ADR-107 Phase 2 (backup/restore with progress tracking), we discovered that progress updates are limited by polling architecture:
...
- ADR-300: Job Queue System
- ADR-107: Backup/Restore Streaming

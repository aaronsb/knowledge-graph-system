...
- **Relationship Types**: Vocabulary embeddings for grounding calculations (ADR-808)
- **Images**: Visual embeddings (Nomic Vision v1.5, 768-dim, ADR-305)
...
4. **Visual Embeddings**: Image similarity search (✅ ADR-305)
5. **Graph Embeddings**: Concept and edge embeddings (✅ ADR-808, ADR-809)
...
#### Grounding Layer (Evaluative - ADR-808, ADR-811)
```
...
- **Algorithm:** Polarity Axis Triangulation (ADR-811)
...
**This ADR addresses evidence retrieval only.** Grounding calculation is handled separately by ADR-808 (Probabilistic Truth Convergence) and ADR-811 (Polarity Axis Triangulation).
...
Leverage existing job system (ADR-300) for embedding generation:
...
def run_source_embedding_worker(
    ...
    3. Generate embeddings via EmbeddingWorker (ADR-809)
...
3. **Vocabulary**: `kg_api.vocabulary_embeddings` table (ADR-808)
...
**Commits:**
1. `feat(mcp): add source search tool with type parameter (ADR-812 Phase 5)` - 0744e837
2. `feat(web): add SourceSearchBlock for source text search (ADR-812 Phase 5)` - 7dfe0b8b
...
With visual embeddings (ADR-305), blend text + visual semantics:
...
- **ADR-600**: Semantic Relationship Taxonomy (Porter stemmer hybrid chunking with overlap)
- **ADR-808**: Probabilistic Truth Convergence (relationship embeddings for grounding)
- **ADR-809**: Unified Embedding Generation (EmbeddingWorker architecture)
- **ADR-305**: Multimodal Image Ingestion (visual embeddings for images)
- **ADR-300**: Job Approval Workflow (async job processing)
- **ADR-804**: Local Embedding Service (embedding configuration system)

...
With the introduction of local inference capabilities (ADR-806: Ollama for extraction, ADR-804: sentence-transformers for embeddings), the system can now run entirely on local GPU hardware. However, this creates resource contention on single-node deployments where both models compete for limited VRAM.
...
- **ADR-804:** Local Embedding Service (introduces sentence-transformers)
- **ADR-806:** Local LLM Inference (introduces Ollama for extraction)

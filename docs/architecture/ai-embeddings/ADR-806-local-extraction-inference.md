...
```json
{
  ...
  "relationships": [{
    ...
    "relationship_type": "IMPLIES",  // 30-90 dynamic types (ADR-601)
    "confidence": 0.9
  }]
}
```

**Dynamic Vocabulary Challenge** (ADR-601):
...
Follow the same pattern established for embeddings (ADR-804):
...
Extend `ai_extraction_config` table (follows embedding pattern from ADR-804):
...
- **ADR-804:** Local Embedding Service - Similar architectural decision for embeddings
- **ADR-805:** AI Extraction Config - Existing config system this extends
- **ADR-601:** Dynamic Relationship Vocabulary - Variable prompt size requirement
- **ADR-210:** Database Schema Migrations - Migration 007 for new config fields

...
3. **Inconsistent with embeddings**: Embeddings use database-first configuration (ADR-804)
...
6. **Split architecture**: API keys in database (ADR-405), but config in .env

### Current Architecture (Split)

```
API Keys (ADR-405)               Configuration (Current)
┌────────────────────┐          ┌────────────────────┐
...
Implement **database-first AI extraction provider configuration**, following the same pattern as ADR-804 (Local Embedding Service).
...
3. **Consistency with Embeddings**
   - Same configuration pattern as `embedding_config` (ADR-804)
...
def get_provider(provider_name: Optional[str] = None) -> AIProvider:
    ...
    if provider_name:
        ...
        # 2. Try database configuration (ADR-805)
...
4. **✅ Consistent**: Same pattern as embedding configuration (ADR-804)
...
1. **Database-first**: Matches embedding configuration approach (ADR-804)
...
**Rejected**: Inconsistent with ADR-804 (embeddings), requires restart for changes
...
API keys stored in `system_api_keys` (ADR-405) are **shared resources** used by multiple systems:

```
system_api_keys (encrypted, ADR-405)
├── openai: sk-...
...
**Key insight:** Local embeddings (ADR-804) don't require API keys, so the embedding worker **skips API key lookup entirely** when `provider='local'`.

### Configuration Independence

```
┌─────────────────────────────────────────────┐
│ Shared: system_api_keys (ADR-405)          │
...
**Enhanced endpoints from ADR-405 with validation status:**
...
- Related: ADR-405 (Encrypted API Key Storage) - Shared API keys
- Related: ADR-804 (Local Embedding Service) - Parallel embedding configuration
- Related: ADR-210 (Database Schema Migrations) - Schema evolution

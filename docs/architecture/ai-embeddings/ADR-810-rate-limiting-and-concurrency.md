...
Following ADR-805 pattern, configuration stored in database with environment fallbacks:
...
- Consistent with ADR-805 pattern
...
Unlike ADR-805 (provider/model selection), rate limiting loads database-first regardless of `DEVELOPMENT_MODE`:
...
- **ADR-805:** AI Extraction Provider Configuration (database-first pattern)

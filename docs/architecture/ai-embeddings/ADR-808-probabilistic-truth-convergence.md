...
### The ADR-808/045/046 Trio

This ADR is part of a three-part system for truth convergence in the knowledge graph:

| ADR | Focus | Purpose |
|-----|-------|---------|
| **ADR-808** | **Theory** | Probabilistic truth convergence through grounding strength |
| **ADR-809** | **Storage** | Unified embedding generation infrastructure |
| **ADR-604** | **Management** | Vocabulary lifecycle with grounding awareness |

**Implementation Order:** ADR-809 → ADR-808 → ADR-604
...
- Evidence: Current codebase (`age_client.py`), ADR-208 migration decision
...
**Update 2025-10-25:** This ADR now depends on **ADR-809 (Unified Embedding Generation)**, which must be implemented first to enable embedding-based grounding calculation.
...
3. Builtin types lack embeddings (0/30 have embeddings - solved by ADR-809)
4. Hard-coding edge polarity doesn't scale with vocabulary expansion (ADR-603)
...
Agent provides context:
  "Given evidence that:
   ...
   - ADR-208 states migration occurred 2025-10-09
...
It previously used Neo4j but migrated in October 2025 per ADR-208."
```
...
- ADR-808 principle: "Always current: Reflects latest edge weights at query time"
...
```json
{
  "interpretation": "Temporal analysis shows Neo4j references end 2025-10-08. ADR-208 explicitly documents migration to Apache AGE on 2025-10-09. Current codebase (age_client.py) and all recent documentation reference Apache AGE. Neo4j represents historical state, not current architecture.",
...
- New ADR: "ADR-308: Neo4j Enterprise for Multi-Region HA"
...
- **ADR-601:** Dynamic relationship vocabulary - establishes CONTRADICTS edge type
- **ADR-502:** Concept deduplication - prevents duplicate concepts, complements contradiction resolution
- **ADR-603:** Confidence thresholds - similar statistical approach for relationship confidence
- **ADR-902:** Node fitness scoring - early concept quality metrics
...
- **ADR-811:** Polarity Axis Triangulation - Replaces binary classification with geometric projection for nuanced grounding percentiles
...
**BLOCKED:** Requires ADR-809 (Unified Embedding Generation) to be implemented first.

**ADR-809 Prerequisites:**
...
**Next Steps:**
1. **Implement ADR-809 first** (unified embedding generation)
...
**Last Updated:** 2025-10-25 (Added ADR-809 dependency and embedding-based approach)
**Next Review:** After ADR-809 implementation

...
- ADR-907 documents full-scan as "genuinely unusual" architectural choice
...
- **ADR-106 (API Server):** Embedding endpoint extends FastAPI server architecture
- **ADR-707 (Unified Client):** kg CLI extended with embedding migration commands
- **ADR-208 (Apache AGE):** Embedding config stored in PostgreSQL alongside graph
- **ADR-502 (Concept Deduplication):** Quality tests must account for model changes
- **ADR-710 (Graph Visualization):** Browser embeddings enhance interactive search
- **ADR-907 (O(n) Full-Scan Search):** Local embeddings compatible with current search architecture

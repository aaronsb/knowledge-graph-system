...
**Location**: `api/app/services/vocabulary_manager.py:871-964` (ADR-111, implemented 2025-10-31)
...
**Future**: Automatic triggers (ADR-111 scheduled jobs)
```python
...
### 5. Grounding-Aware Vocabulary Classification (ADR-808 Integration)
...
for vocab_type in active_vocabulary:
    ...
    # Calculate grounding profile (ADR-808)
    groundings = [
...
for vocab_type in vocabulary:
    ...
    # Store as vocabulary metadata (ADR-606)
...
- **ADR-808**: Probabilistic Truth Convergence (provides grounding_strength metric)
- **ADR-809**: Unified Embedding Generation (enables grounding calculation)
- **ADR-604**: Grounding-Aware Vocabulary Management (usage tracking with quality metrics)
...
- **ADR-601**: Dynamic Relationship Vocabulary (foundation for vocabulary expansion)
- **ADR-603**: Automatic Edge Vocabulary Expansion (optimistic generation strategy)
- **ADR-604**: Grounding-Aware Vocabulary Management (usage tracking)
- **ADR-605**: Probabilistic Vocabulary Categorization (semantic classification)
- **ADR-111**: Scheduled Jobs System (automation framework for consolidation)

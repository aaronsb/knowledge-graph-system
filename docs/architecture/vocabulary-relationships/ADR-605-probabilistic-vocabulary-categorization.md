...
Implement **probabilistic category assignment** using embedding similarity - the same pattern that succeeded with grounding strength (ADR-808).
...
**Integrated Workflow with ADR-603 (Pruning) and ADR-604 (Synonym Detection):**
...
# 2. FIND pruning candidates (ADR-605 categories reveal)
...
| Insight | Action | ADR |
|---------|--------|-----|
| **Orphans** (confidence < 50%, low edges) | Deprecate weak types | ADR-603 |
| **Synonyms** (same category, similarity > 0.85) | Merge redundant types | ADR-604 |
| **Imbalances** (40 causation, 3 temporal) | Need better seed diversity | ADR-605 |
| **Bridges** (ambiguous, runner-up > 0.70) | Keep valuable connectors | ADR-605 |
...
- **ADR-808:** Probabilistic Truth Convergence (pattern we're following)
- **ADR-603:** Automatic Edge Vocabulary Expansion (pruning weak types)
- **ADR-604:** Grounding-Aware Vocabulary Management (synonym detection via embeddings)
- **ADR-601:** Dynamic Relationship Vocabulary (allows new types)
- **ADR-600:** Semantic Relationship Taxonomy (defined the 8 categories)

---

**This ADR continues the evolution from fixed → probabilistic systems:**
1. Fixed relationships (5 types) → Dynamic vocabulary (ADR-601)
2. Fixed truth → Probabilistic grounding (ADR-808)
3. Fixed categories → Probabilistic categorization (ADR-605) ✨

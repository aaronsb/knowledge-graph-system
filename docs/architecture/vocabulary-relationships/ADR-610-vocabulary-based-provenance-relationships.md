...
# - Polarity axis triangulation (ADR-811) for grounding
...
Through successive design decisions—vocabulary expansion-consolidation (ADR-607), polarity axis triangulation (ADR-811), and now vocabulary-based provenance—we've been **systematically eliminating boolean logic in favor of probabilistic values**.
...
1. **ADR-607 (Vocabulary Expansion-Consolidation):** Instead of hardcoded relationship types, use similarity-based clustering
   - Boolean: "Is this SUPPORTS?" → Probabilistic: "87% similar to SUPPORTS"

2. **ADR-811 (Polarity Axis):** Instead of binary support/contradict, project onto axis
   - Boolean: "Supporting or contradicting?" → Probabilistic: "+0.73 projection (leaning support)"

3. **ADR-610 (Vocabulary-Based APPEARS):** Instead of binary presence, use appearance strength
...
- pgvector for native PostgreSQL similarity search (ADR-306)
...
**Treat APPEARS as a semantic prototype in embedding space**, identical to how SUPPORTS/CONTRADICTS function as polarity prototypes in ADR-811.
...
Apply ADR-811's polarity axis triangulation to appearance relationships:
...
**Vocabulary-aware source matching** (ADR-606 pattern):
...
class GraphQueryFacade:
    ...
    def match_concept_sources(
        ...
        if strength_threshold:
            # Filter by projection on appearance_axis (ADR-811 style)
...
class AGEClient:
    ...
    def calculate_appearance_strength(
        ...
        """
        Calculate edge position on appearance strength axis (ADR-811).
        ...
        # Same algorithm as grounding calculation (ADR-811)
...
- **ADR-811:** Polarity Axis Triangulation for Grounding Calculation (pattern we're replicating)
- **ADR-606:** Query Safety via GraphQueryFacade (abstraction layer for vocabulary complexity)
- **ADR-607:** Vocabulary Expansion-Consolidation Cycle (applies to appearance types)
...
- Vocabulary consolidation: ADR-607

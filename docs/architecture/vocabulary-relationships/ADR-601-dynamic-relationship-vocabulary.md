...
From ADR-904 (Pure Graph Design):
...
### Integration with ADR-209

Add to `kg_api` schema in ADR-209:
...
- ADR-209: Add vocabulary tables to `kg_api` schema
...
- ADR-904: Pure Graph Design (original vocabulary rationale)
- ADR-209: Multi-Schema PostgreSQL Architecture (database infrastructure)

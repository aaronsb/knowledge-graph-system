...
# ADR-603.1: Automatic Edge Vocabulary Expansion with Intelligent Pruning
...
The current system uses a static 30-type relationship vocabulary defined in `api/app/constants.py`. While ADR-601 and ADR-602 propose dynamic vocabulary management with manual curator approval, this creates a bottleneck during high-volume ingestion.

### Current Limitations

**Static Vocabulary (ADR-600):**
```python
RELATIONSHIP_TYPES = {
...
**ADR-601 Proposed Flow (Not Implemented):**
```
...
> **Note:** Category classification is now handled by **ADR-605: Probabilistic Vocabulary Categorization** using embedding similarity to seed types with satisficing (max similarity). The approach below is superseded.

**Two-Tier Vocabulary Structure:**

```python
# High-level categories (8 protected groups from ADR-600, refined in ADR-605)
...
**Problem:** Initial backup system (ADR-107) only exported graph data:
```json
{
  "data": {
    "concepts": [...],
...
Because ADR-603 structures vocabulary as **managed state** (not just emergent properties), backups become snapshots of TWO things:
...
def import_backup(backup_data):
    ...
    # 1. Import vocabulary FIRST (ADR-603)
...
- `api/app/lib/backup_integrity.py`: Added vocabulary section validation
...
### 1. Manual Approval for Every Type (ADR-601)
...
- **ADR-600:** 30-Type Semantically Sparse Taxonomy (current static system)
- **ADR-601:** Dynamic Relationship Vocabulary (skip-and-approve workflow)
- **ADR-602:** Autonomous Vocabulary Curation (LLM-assisted suggestions)
- **ADR-605:** Probabilistic Vocabulary Categorization (embedding-based category assignment)
- **ADR-604:** Grounding-Aware Vocabulary Management (synonym detection, compaction workflow)
- **ADR-300:** Job Approval Workflow (HITL pattern)
- **ADR-110:** Live Man Switch (human oversight principles)

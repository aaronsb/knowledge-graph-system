...
LLMs extract relationships with directional ambiguity. From extraction quality comparison (ADR-806 testing):
...
From ADR-605/ADR-606 implementation:

| Property | Storage | Granularity | Status |
|----------|---------|-------------|---------|
| **category** | :VocabType nodes + edges | Per type + per relationship | ✅ Complete (ADR-606) |
...
**Parallel to ADR-605 (Probabilistic Categorization):**

| Aspect | ADR-605 Categories | ADR-810 Direction |
|--------|-------------------|-------------------|
...
**Rejected:**
- Violates dynamic vocabulary principle (ADR-601)
- Can't handle custom types like "COMPETES_WITH"
- Breaks emergent pattern learning
- Inconsistent with ADR-605 (emergent categories)
...
- **ADR-605:** Probabilistic Vocabulary Categorization (emergent categories)
- **ADR-606:** Vocabulary Metadata as First-Class Graph
- **ADR-601:** Dynamic Relationship Vocabulary (allows custom types)
- **ADR-600:** Semantic Relationship Taxonomy (30 seed types)
- **ADR-806:** Local LLM Inference (extraction quality comparison showing 35% direction errors)
...
1. **Category** (computed via embeddings, ADR-605)

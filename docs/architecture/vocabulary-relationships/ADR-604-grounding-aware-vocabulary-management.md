...
As the system operates, vocabulary expands through LLM-generated edge types (ADR-603). While this enables semantic richness, unchecked growth creates **dilution risks**:
...
### Current State: ADR-603 Sliding Window
...
### How ADR-808/045 Enable Better Management
...
Extend ADR-603 with **grounding contribution metrics** and **embedding-based synonym clustering** to:
...
def find_synonym_clusters(
    ...
    Uses semantic embeddings (ADR-809) instead of string matching.
...
def detect_synonyms_semantic(
    ...
    """
    Detect synonyms using embedding similarity (ADR-809).
...
Update `merge_edge_types()` to handle embeddings (per ADR-809):
...
✅ **Future-proof** - Works with any embedding model (ADR-809)
...
⚠️ **Requires ADR-808/045** - Cannot implement independently
...
ADR-808 (Grounding)
    ↓
ADR-604 (Management) ← This ADR
```

**Extends:**
- **ADR-603:** Automatic Edge Vocabulary Expansion (adds grounding awareness)

**Enables:**
- **ADR-808:** Probabilistic Truth Convergence (protects grounding from dilution)

**Requires:**
- **ADR-809:** Unified Embedding Generation (for semantic similarity)
...
**Prerequisites:**
- [ ] ADR-809 Phase 1: EmbeddingWorker (must complete first)
- [ ] ADR-808 Phase 1: Grounding calculation (must complete first)

**ADR-604 Implementation:**
...
**Next Steps:**
1. Complete ADR-809 (embeddings) and ADR-808 (grounding) first
...
5. Integrate with ADR-605 category-based synonym detection
6. Test with production vocabulary (64 types)

## References

- **ADR-808:** Probabilistic Truth Convergence (grounding strength calculation)
- **ADR-809:** Unified Embedding Generation (embeddings for all vocabulary types)
- **ADR-603:** Automatic Edge Vocabulary Expansion (pruning weak types)
- **ADR-605:** Probabilistic Vocabulary Categorization (category-based synonym detection)
- **ADR-600:** Semantic Relationship Taxonomy (8 categories)

---

**Last Updated:** 2025-10-27
**Next Review:** After ADR-808/045/047 implementation

...
After completing ADR-605, we recognized a fundamental architectural mismatch:
...
// Categorization relationship (from ADR-605)
...
// Synonym relationship (from ADR-604)
...
// Refresh categories after merge (ADR-605 workflow)
...
- "Vocabulary is part of the timeless graph" (ADR-605) becomes literal
...
- **ADR-605:** Probabilistic Vocabulary Categorization (categories emerge from embeddings)
- **ADR-604:** Grounding-Aware Vocabulary Management (synonym detection)
- **ADR-603:** Automatic Edge Vocabulary Expansion (pruning weak types)
- **ADR-904:** Pure Graph Design (graph stores knowledge, not business logic)
...
3. **Better categorization** (ADR-605 probabilistic approach)

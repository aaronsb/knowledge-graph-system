...
When you want to analyze where concepts fall on a semantic spectrum (using polarity axis analysis from ADR-813), you first need to know what spectrum to measure. Currently, this requires conceptual guesswork: "Maybe Modern↔Traditional is an interesting axis?" You're imposing structure before you've seen the data.
...
- Embedding regeneration (ADR-812)
...
**Related work:**
This overlaps with a broader consideration for storing original source documents (not just images) in Garage. Currently only images are stored in object storage (ADR-305). A future ADR should address:

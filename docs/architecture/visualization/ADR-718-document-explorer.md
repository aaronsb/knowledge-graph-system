...
# ADR-718: Document Explorer with Radial Concept Visualization

## Context

ADR-507 introduced document-level search APIs. The next step is visualizing document→concept relationships in the web UI. Users need to understand:
...
- **ADR-808** - bounded locality with depth=1 default, max 3 hops
...
Follow ADR-710 Explorer Plugin Interface:
...
- **Familiar patterns** - follows ADR-710 explorer interface
...
- **ADR-808:** Probabilistic Truth Convergence - bounded locality, satisficing, grounding calculation
- **ADR-507:** Document-Level Search - APIs for document discovery

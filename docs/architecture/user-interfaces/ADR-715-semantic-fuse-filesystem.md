...
See ADR-715.1 for detailed implementation specifics (phases, caching, query store).
...
But knowledge doesn't fit in trees. ADR-812 is simultaneously:
...
- **Our Solution:** Asynchronous worker pattern (ADR-300) with job queue
...
5. **OAuth Client Authentication** - Same auth as CLI and MCP (ADR-406)
...
- ADR-715.1: FUSE Implementation Specifics
- ADR-406: OAuth 2.0 Authentication
- ADR-112: Sharding and facet architecture

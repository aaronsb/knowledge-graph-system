...
# ADR-713: Specialized Truth Convergence Visualizations
...
The current web visualization (ADR-710, ADR-711) provides 2D and 3D force-directed graph explorers that excel at showing connectivity patterns and conceptual neighborhoods. However, the platform has unique capabilities that are not leveraged by traditional node-edge visualizations:

**Unique Platform Capabilities:**
- **Truth convergence:** Grounding strength (-1.0 to 1.0) measures reliability vs. contradiction (ADR-808, ADR-811)
- **Semantic diversity:** Measures conceptual richness via percentage of diverse connections (ADR-503)
- **Provenance tracking:** DocumentMeta nodes, relationship audit trails (ADR-304)
- **Relationship polarity:** Support vs. contradict indicators with authenticated diversity
- **Vocabulary lifecycle:** Dynamic relationship types with categorization (ADR-607, ADR-608)
...
All new explorers follow the **plugin pattern** established in ADR-710:
...
- ✅ Grounding strength (`grounding_strength` on relationships - ADR-808, ADR-811)
- ✅ Semantic diversity (calculated on-demand - ADR-503)
- ✅ Provenance tracking (`DocumentMeta` nodes, `source_type`, `ingested_by` - ADR-304)
- ✅ Relationship polarity (embedded in polarity axis projection - ADR-811)
- ✅ Vocabulary categories (`VocabType`, `VocabCategory` - ADR-605, ADR-608)
...
'Tracks vocabulary expansion-consolidation cycle (ADR-607) for timeline visualization';
```
...
2. **Polarity Spectrum** (2D)
   - Uses: Relationship polarity from ADR-811 projection
...
11. **Vocabulary Lifecycle Timeline** (2D)
    ...
    - Value: Track expansion-consolidation cycle (ADR-607)
...
- **ADR-710:** Establishes web visualization architecture and force graph explorers
- **ADR-711:** Documents interaction patterns and "You Are Here" navigation
- **ADR-808:** Probabilistic truth convergence provides grounding strength metric
- **ADR-304:** Graph provenance tracking enables DocumentMeta → Concept flows
- **ADR-607:** Vocabulary expansion-consolidation cycle informs lifecycle visualization
- **ADR-811:** Polarity axis triangulation provides relationship polarity for spectrum view
- **ADR-503:** Semantic diversity as authenticity signal drives diversity visualizations

...
- **ADR-208**: Apache AGE Migration - The foundation that enables O(n) full scan
- **ADR-502**: Concept Deduplication Validation - Quality test suite that validates the approach
- **DISTRIBUTED_SHARDING_RESEARCH.md**: The 1,000-line document that proves we know this doesn't scale (and how to fix it)
- **ADR-712**: Universal Visual Query Builder - The UI that makes the graph actually usable
- **ADR-303**: Human-Guided Graph Editing - Future feature for when machines aren't enough
...
If this project is the Wes Anderson version of graph memory, then ADR-907 isn't just a design document—it's a **costume department memo**. It explains why we have 88+ ADRs: Wes Anderson doesn't just "film a scene," he specifies the exact shade of saffron for the curtains and the kerning on the telegram.

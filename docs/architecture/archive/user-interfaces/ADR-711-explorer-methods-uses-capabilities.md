...
Building a graph visualization isn't enough—you need to make it usable. ADR-710 established the architecture for explorers, but left crucial questions unanswered: How do users navigate between concepts? What happens when they click a node? How do they follow relationships without losing their place?
...
ADR-710 establishes the core architecture for the graph visualization application. This ADR documents the specific explorer types, their use cases, interaction patterns, and planned enhancements for navigating and understanding the knowledge graph.
...
**Planned Interactions (ADR-711 Enhancements):**
...
For **Edit Mode** context menu actions (create connections, invalidate relationships), see **ADR-303: Human-Guided Graph Editing**.
...
- ADR-710: Graph Visualization Architecture

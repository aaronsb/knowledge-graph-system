...
The current RBAC system (ADR-404) provides four builtin roles:
...
The RBAC system (ADR-404) is already fully flexible. This ADR adds the missing pieces to make it useful for platform administration.
...
```sql
-- Migration: Platform Admin Resources and Role (ADR-409)
...
- [ADR-403: Authentication](ADR-403-user-management-api.md) - User authentication
- [ADR-404: Dynamic RBAC System](./ADR-404-dynamic-rbac-system.md) - Core RBAC implementation
- [ADR-405: API Key Management](ADR-405-encrypted-api-key-storage.md) - Encrypted API key storage

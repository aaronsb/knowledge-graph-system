...
The knowledge graph system currently has user authentication (ADR-404) but lacks:
...
3. **Specific artifact storage** - See ADR-116 for polarity reports
...
2. Update projection storage (ADR-114) to include owner
...
- ADR-404: Dynamic RBAC (existing role system)
- ADR-114: Projection Artifact Storage (Garage pattern)

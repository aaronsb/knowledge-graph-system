...
Initial scope set (aligned with RBAC resources from ADR-404):
...
1. **RBAC Integration**
   ...
   - No changes to RBAC system (ADR-404)
...
**Related:**
- ADR-403: User Management API (superseded for OAuth)
- ADR-404: Dynamic RBAC System (unchanged)
- ADR-209: Multi-Schema PostgreSQL Architecture (kg_auth schema)

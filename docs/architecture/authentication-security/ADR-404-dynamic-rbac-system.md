...
The current authentication system (ADR-403) has hardcoded roles (`read_only`, `contributor`, `curator`, `admin`) with static permissions seeded in `kg_auth.role_permissions`. As the platform evolves to support:
...
**Permissions Seeded:**
- All existing permissions from ADR-403 migrated automatically

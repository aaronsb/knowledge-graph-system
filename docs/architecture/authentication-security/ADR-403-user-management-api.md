...
**See ADR-406** for the current OAuth 2.0 implementation which provides:
...
The `kg_auth` schema (ADR-209) provides the foundation with users, API keys, OAuth tokens, and role permissions tables. We need to implement REST API endpoints for user management that leverage this existing schema.
...
### ~~Future OAuth Integration (Phase 2)~~ → Superseded by ADR-406

**⚠️ This section described a plan for external OAuth providers (Login with GitHub/Google). This has been superseded by ADR-406 which implements:**
...
- This is orthogonal to ADR-406 (our OAuth server)
- Can be implemented later if needed

**See ADR-406** for complete OAuth 2.0 architecture.
...
### Related ADRs
- ADR-209: Multi-Schema PostgreSQL Architecture (defines `kg_auth` schema)
- ADR-601: Dynamic Relationship Vocabulary (permission model usage)
- ADR-602: Autonomous Vocabulary Curation (curator role integration)

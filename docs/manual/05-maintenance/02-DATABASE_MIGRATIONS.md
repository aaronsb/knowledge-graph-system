...
- `./operator/database/migrate-db.sh` applies pending changes automatically
...
↓
Automatically runs ./operator/database/migrate-db.sh -y
↓
...
2. Run `./operator/database/migrate-db.sh -y` again
...
```bash
./operator/database/migrate-db.sh              # Interactive (asks for confirmation)
./operator/database/migrate-db.sh -y           # Auto-confirm (for CI/CD)
./operator/database/migrate-db.sh --dry-run    # Preview only (no changes)
./operator/database/migrate-db.sh -v           # Verbose (show SQL)
./operator/database/migrate-db.sh --help       # Show usage
```
...
```bash
kg admin reset             # destructive: drops all data, requires admin auth
# OR (operator-level teardown)
./operator.sh teardown     # removes containers/volumes; follow with ./operator.sh start
```

...
│
├── kg_auth             # Security and access control (ADR-028, ADR-054)
│   ├── users                   # User accounts (primary_role + bcrypt password_hash)
│   ├── roles                   # Dynamic role definitions with inheritance
│   ├── user_roles              # Users ↔ additional roles
│   ├── resources               # Dynamic resource type registry
│   ├── permissions             # Role → resource action grants
│   └── oauth_clients           # OAuth 2.0 client credentials (ADR-054)
│
...
The admin user is created interactively by `./operator.sh init` — there is no shared default password. To reset, use `./operator/admin/reset-password.sh` (see [04-PASSWORD_RECOVERY.md](../04-security-and-access/04-PASSWORD_RECOVERY.md)).

The admin user's `primary_role` is set to `admin` (full system access).
...
| `admin` | Full system access | Curator + manage users/roles, cancel any job |
| `platform_admin` | Platform operator | Admin + system job management |

Roles can inherit from a parent role (`kg_auth.roles.parent_role`). See [04-security-and-access/02-RBAC.md](../04-security-and-access/02-RBAC.md) for the full permission matrix.
...
- `primary_role` (VARCHAR, FK → `kg_auth.roles.role_name`) — `read_only` | `contributor` | `curator` | `admin` | `platform_admin`
- Additional roles can be assigned via `kg_auth.user_roles`.

**Password Security:**
- Bcrypt hashed passwords (cost 12)
- No plaintext storage
- OAuth 2.0 client credentials for API requests (ADR-054)
...
**Port:** `5432`
**User:** `admin`
**Password:** generated by `./operator.sh init` and stored in `.env` as `POSTGRES_PASSWORD`

**Connection String:**
```
postgresql://admin:$POSTGRES_PASSWORD@localhost:5432/knowledge_graph
```

Most users should not query PostgreSQL directly — use the `kg` CLI or the API. For ad-hoc Cypher use `kg database query "<cypher>"` (ADR-048).
...
```bash
docker exec knowledge-graph-postgres psql -U admin -d knowledge_graph -c "
SELECT username, primary_role FROM kg_auth.users WHERE primary_role = 'admin';
"
# Expected: admin user created by ./operator.sh init
```
...
```bash
# Reset to fresh baseline (destructive; requires admin auth)
kg admin reset

# Restore your data (use a kg admin backup file)
kg admin restore --file your-backup.json
...
# Check user role
docker exec knowledge-graph-postgres psql -U admin -d knowledge_graph -c "
SELECT username, primary_role FROM kg_auth.users WHERE username = 'your_user';
"

# Check role permissions
docker exec knowledge-graph-postgres psql -U admin -d knowledge_graph -c "
SELECT * FROM kg_auth.permissions WHERE role_name = 'your_role';
"
```

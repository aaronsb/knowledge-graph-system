...
1. **Encryption at Rest**
   ...
   - Stored as binary blobs in PostgreSQL (`kg_api.system_api_keys`)
...
**Prerequisites:**
1. Platform running: `./operator.sh start` (brings up PostgreSQL + API in containers)
2. Encryption master key configured (auto-generated by `./operator.sh init`; for production override via Docker secret or `ENCRYPTION_KEY`)
...
# Option 2: Set in .env (legacy/development mode)
# Note: .env is generated by ./operator.sh init; in production prefer encrypted storage above.
./operator.sh restart api  # restart to load new env
```
...
- `kg_api.system_api_keys` table doesn't exist
...
- **[../../guides/QUICKSTART.md](../01-getting-started/02-CLI_USAGE.md)** - Initial system setup

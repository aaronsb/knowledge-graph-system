-- ============================================================================
-- VENDORED FROM UPSTREAM APACHE AGE -- DO NOT EDIT
-- ============================================================================
-- Source:      https://github.com/apache/age/blob/PG18/v1.8.0-rc0/age--1.7.0--1.8.0.sql
-- Upstream tag: PG18/v1.8.0-rc0
-- Upstream path: age--1.7.0--1.8.0.sql (repository root)
-- Git blob:    fe4c9a5f2577053d8ba11fb77ad201fe4c39a68a
-- sha256:      9b04323cc00cd57a9d28adc43c01f5de43d236c26d3e1a17682bc3ce039e35af
-- Fetched:     2026-09-17
--
-- WHY WE CARRY THIS FILE
-- ---------------------
-- Upstream's Makefile installs every age--*--*.sql update script:
--     DATA = $(filter-out age--%--y.y.y.sql $(age_upgrade_test_sql),$(wildcard age--*--*.sql))
-- but upstream's docker/Dockerfile enumerates the files it copies out of the
-- build stage and lists only three:
--     COPY --from=build .../age.so                  ...
--     COPY --from=build .../age--1.8.0.sql          ...
--     COPY --from=build .../age.control             ...
-- so the published apache/age images ship NO update scripts at all (`find / -name
-- 'age--*'` in release_PG18_1.8.0 returns only age--1.8.0.sql; same for 1.7.0).
--
-- Without this file `ALTER EXTENSION age UPDATE` fails with "no update path from
-- version 1.7.0 to version 1.8.0", which leaves an existing cluster running the
-- 1.8.0 library against a 1.7.0 catalog. Writes still succeed but every read
-- fails with "type with OID 0 does not exist" -- three functions the executor
-- uses to materialise rows were re-signatured in 1.8.0 (_agtype_build_vertex and
-- _agtype_build_edge take agtype where they took cstring; _label_name RETURNS
-- agtype where it returned cstring).
--
-- docker/Dockerfile.postgres COPYs this into the extension directory next to
-- age--1.8.0.sql, and operator/lib/start-infra.sh runs the update on start.
--
-- Everything below this header is byte-identical to upstream. To re-verify:
--   tail -n +41 docker/age/age--1.7.0--1.8.0.sql | sha256sum
--   # => 9b04323cc00cd57a9d28adc43c01f5de43d236c26d3e1a17682bc3ce039e35af
--
-- Upstream report: https://github.com/apache/age/issues/2570
-- ============================================================================
